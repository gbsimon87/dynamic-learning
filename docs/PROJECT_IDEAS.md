...
| 4 | **Automated component test setup** | Add Vitest + React Testing Library and cover the highest-risk React logic first: unlock rules, progress persistence, and the challenge loader. Pure logic has dependency-free `node:test` coverage, but no component test environment exists. Include `src/data/store/apiStore.js`, which has no `fetch`-mocked tests despite being the live data path. | ⚪ Idea |
