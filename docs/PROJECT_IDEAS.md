...
- Curriculum Mode has challenges for **30 of 39 Year 2 Maths topics** — every
  Number category, Measurement, and Geometry – Properties of Shapes. The
  remaining 9 are Geometry – Position and Direction, and Statistics.
...
| 20 | **Complete Year 2 Maths curriculum** | Build challenges 1–4 for the remaining 9 Year 2 Maths topics (place value, addition/subtraction, multiplication/division, fractions, measurement, geometry, statistics). Biggest single body of work; unlocks the whole curriculum path. The four Number categories were completed 2026-09-18 on the shared challenge kit (`src/components/challenge/`); follow the `building-curriculum-topics` skill, composing the kit rather than copying a challenge and extending it when a topic needs an interaction it lacks. | 🟡 Partial (30/39 topics — Number, Measurement and shape properties complete) |
