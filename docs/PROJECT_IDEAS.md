...
| 2 | **Progress visibility (per year, per topic)** | Partly shipped. The homepage shows a ring for the **current topic** plus "X of Y challenges done" and a resume card (`useHomeResume` / `homeResume.js`), and `/curriculum/year/:year/:subject` shows a year ring, a per-category bar and per-topic counts. What is still missing is everything outside those two screens: **`/profiles` cards show no progress at all**, so a grown-up choosing between children sees nothing, and there is no per-category or per-topic breakdown when a profile is opened. Read through `useProgress` and `curriculumProgressStats.js` rather than touching storage directly. Grows into a fuller "my journey" view. | 🟡 Partial |
| 3 | **Rewards & motivation system** | Badges, streaks, and unlockable avatars awarded on challenge and topic completion — the persistent layer beneath the shipped momentary celebrations (`completionMilestones.js`). Hooks into the same `onComplete` contract. | ⚪ Idea |
...
| 8 | **Content authoring format** | Move challenge question data out of hand-written JSX into declarative JSON/JS content files, so new challenges can be authored without writing a component each time. The per-topic generator modules in `src/data/challenges/` are a step in this direction already. Makes #18 and future curriculum years substantially cheaper. | ⚪ Idea |
| 9 | **Accessibility & keyboard pass** | Keyboard alternatives for every drag-and-drop challenge, ARIA labels, focus management, and a colour-contrast audit across both themes. **One of 39 kit components handles keys at all** (`MeasureDrag`), so every other drag interaction is mouse- and touch-only. Contrast has been measured per-screen as pages were built, but never swept as a whole. | ⚪ Idea |
| 10 | **Audio & read-aloud support** | Speech synthesis for questions and instructions, so pre-readers aren't blocked by reading ability. `SolarSystem.jsx` is the only place `speechSynthesis` is used today, and it is a fair template. Also the foundation for #14. | ⚪ Idea |
| 11 | **Parent / teacher summary view** | A summary screen showing time spent, topics mastered, topics struggled with, and a reset-progress control. `/parent` exists and handles profile management and sign-out, so this has a home to grow into rather than needing a new route — but it shows no progress data of any kind today. Read-only apart from the reset. Distinct from #2, which is learner-facing. Note a learner account sees this screen too, titled "My account". | ⚪ Idea |
| 12 | **Adaptive difficulty** | Track accuracy and response time per topic, then adjust number ranges and question difficulty automatically instead of using fixed random ranges. The pure-generator pattern in `src/data/challenges/` is what makes this tractable — difficulty is already a parameter in most generators. | ⚪ Idea |
| 13 | **Verifiable parental consent** | The under-13 gate on signup is an honest speed bump, not a compliance implementation: a child can simply tap an older age. Fine for a family project; **not sufficient if this is ever published**, where UK AADC and COPPA both expect verifiable consent. Would build on #5's email transport: a grown-up's address, a confirmation round trip, and a record of it on the account. Revisit before any public launch, not before. | ⚪ Idea |
| 14 | **Phonics & rhyming pack** | Sound-it-out, rhyming match, and syllable splitter. Requires the audio work in #10. Strengthens the English side, which is currently Skills-only. | ⚪ Idea |
| 15 | **Story Maker & Story Sequencer** | Pick characters/setting/action to generate a story; arrange picture cards into the right order. Creative writing plus comprehension and sequencing. `stories.json` exists but feeds `SpeedReader` only — it is reading material, not a story-building format, so this needs its own content shape. | ⚪ Idea |
| 16 | **Offline / PWA support** | Service worker plus manifest so the app installs and works without a connection — valuable on shared or low-connectivity school devices. No manifest, service worker, or PWA plugin exists today. Note this interacts with the API store: offline progress would need a queue and a conflict rule. | ⚪ Idea |
| 17 | **Geography curriculum expansion** | Turn the geography backlog (continents, oceans, landforms, habitats, weather, day/night) into a structured topic path rather than loose games. Source ideas in `notes_geography.md`. [Flag Finder](../src/pages/skills/geography/FlagFinder.jsx) uses the REST Countries API and could filter by continent — "Which African country?" — enabling continent-focused topics. | ⚪ Idea |
| 18 | **English curriculum mode** | Extend Curriculum Mode beyond Maths — author a Year 2 English curriculum tree (phonics, spelling, grammar, comprehension) reusing the same category/topic/challenge machinery. Much cheaper after #8. | ⚪ Idea |
...
2. **Close the motivation loop:** #2 (finish progress visibility, starting with
   the `/profiles` cards) → #3 (rewards). Both build on the `onComplete`
   contract and the progress store that the homepage resume already reads.
3. **Unblock quality:** #4 (component tests) and #6 (progress schema migration),
   before more content multiplies the surface area.
4. **Close the account gap:** #5 (password reset). Children now hold their own
   accounts, and there is currently no way back into a locked-out one.
5. **Unblock scale:** #8 (content authoring format) — makes #18 and future
   curriculum years far cheaper.
6. **Broaden and polish:** #7, #9, #10, #11 and beyond.
